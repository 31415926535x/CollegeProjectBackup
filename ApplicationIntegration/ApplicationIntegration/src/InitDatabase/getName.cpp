...
void NAME()
{
	...
	string ME2m[140] = { "伟", "刚", "勇", "毅", "俊", "峰", "强", "军", "平", "保", "东", "文", "辉", "力", "明", "永", "健", "世", "广", "志", "义", "兴", "良", "海", "山", "仁", "波", "宁", "贵", "福", "生", "龙", "元", "全", "国", "胜", "学", "祥", "才", "发", "武", "新", "利", "清", "飞", "彬", "富", "顺", "信", "子", "杰", "涛", "昌", "成", "康", "星", "光", "天", "达", "安", "岩", "中", "茂", "进", "林", "有", "坚", "和", "彪", "博", "诚", "先", "敬", "震", "振", "壮", "会", "思", "群", "豪", "心", "邦", "承", "乐", "绍", "功", "松", "善", "厚", "庆", "磊", "民", "友", "裕", "河", "哲", "江", "超", "浩", "亮", "政", "谦", "亨", "奇", "固", "之", "轮", "翰", "朗", "伯", "宏", "言", "若", "鸣", "朋", "斌", "梁", "栋", "维", "启", "克", "伦", "翔", "旭", "鹏", "泽", "晨", "辰", "士", "以", "建", "家", "致", "树", "炎", "德", "行", "时", "泰", "盛" };
	...
	string ME2f[165] = { "秀", "娟", "英", "华", "慧", "巧", "美", "娜", "静", "淑", "惠", "珠", "翠", "雅", "芝", "玉", "萍", "红", "娥", "玲", "芬", "芳", "燕", "彩", "春", "菊", "兰", "凤", "洁", "梅", "琳", "素", "云", "莲", "真", "环", "雪", "荣", "爱", "妹", "霞", "香", "月", "莺", "媛", "艳", "瑞", "凡", "佳", "嘉", "琼", "勤", "珍", "贞", "莉", "桂", "娣", "叶", "璧", "璐", "娅", "琦", "晶", "妍", "茜", "秋", "珊", "莎", "锦", "黛", "青", "倩", "婷", "姣", "婉", "娴", "瑾", "颖", "露", "瑶", "怡", "婵", "雁", "蓓", "纨", "仪", "荷", "丹", "蓉", "眉", "君", "琴", "蕊", "薇", "菁", "梦", "岚", "苑", "筠", "柔", "竹", "霭", "凝", "晓", "欢", "霄", "枫", "芸", "菲", "寒", "欣", "滢", "伊", "亚", "宜", "可", "姬", "舒", "影", "荔", "枝", "思", "丽", "秀", "飘", "育", "馥", "琦", "晶", "妍", "茜", "秋", "珊", "莎", "锦", "黛", "青", "倩", "婷", "宁", "蓓", "纨", "苑", "婕", "馨", "瑗", "琰", "韵", "融", "园", "艺", "咏", "卿", "聪", "澜", "纯", "毓", "悦", "昭", "冰", "爽", "琬", "茗", "羽", "希" };

	int NA1Roll = rnd() % 444;//单姓
	int NA2Roll = rnd() % 59;//复姓
	int ME1mRoll = rnd() % 140;//男名
	int ME2mRoll = rnd() % 140;//男名
	int ME1fRoll = rnd() % 165;//女名
	int ME2fRoll = rnd() % 165;//女名

	//单姓单名男
	cout << NA1[NA1Roll] << " " << ME1m[ME1mRoll] << " 男" << endl;

	NA1Roll = rnd() % 444;//单姓
	ME1mRoll = rnd() % 140;//男名
	//单姓单名女
	cout << NA1[NA1Roll] << " " << ME1f[ME1fRoll] << " 女" << endl;

	NA1Roll = rnd() % 444;//单姓
	ME1mRoll = rnd() % 140;//男名
	ME2mRoll = rnd() % 140;//男名
	//单姓双名男
	cout << NA1[NA1Roll] << " " << ME1m[ME1mRoll] << ME1m[ME2mRoll] << " 男" << endl;

	NA1Roll = rnd() % 444;//单姓
	ME1fRoll = rnd() % 165;//女名
	ME2fRoll = rnd() % 165;//女名
	//单姓双名女
	cout << NA1[NA1Roll] << " " << ME1f[ME1fRoll] << ME1f[ME2fRoll] << " 女" << endl;
	...
	if(rnd() % 2 == 1)
	{
		...
		ME1mRoll = rnd() % 140;//男名
		//复姓单名男
		cout << NA2[NA2Roll] << " " << ME1m[ME1mRoll] << " 男" << endl;
	}
	else
	{
		NA2Roll = rnd() % 59;//复姓
		ME1fRoll = rnd() % 165;//女名
		//复姓单名女
		cout << NA2[NA2Roll] << " " << ME1f[ME1fRoll] << " 女" << endl;
	}
	if(rnd() % 2 == 1)
	{
		NA2Roll = rnd() % 59;//复姓
		...
		ME2mRoll = rnd() % 140;//男名
		//复姓双名男
		cout << NA2[NA2Roll] << " " << ME1m[ME1mRoll] << ME2m[ME2mRoll] << " 男" << endl;
	}
	else
	{
		...
		ME2fRoll = rnd() % 165;//女名
		//复姓双名女
		cout << NA2[NA2Roll] << " " << ME1f[ME1fRoll] << ME2f[ME2fRoll] << " 女" << endl;
	}
	// cout << "\n" << endl;
